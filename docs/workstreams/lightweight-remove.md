...
`agent/goguma-salad/lightweight-remove-demo`
...
- 사물 종류 선택 UI는 제거하고 사용자가 그린 삭제 영역만으로 실행한다.
- 경량 삭제 완료 후에는 지운 사물 이동 마커를 만들지 않고, AR 복원 patch만 남긴다.
- 작업 도구의 `가구 추가` 탭은 화면을 종료하지 않도록 해, 삭제 도중 카탈로그로 복귀하는 오동작을 막는다.
- ML Kit 초기화와 AR 결과 적용 예외는 화면 종료 대신 상태 문구로 처리한다.
