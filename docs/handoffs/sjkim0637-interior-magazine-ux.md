...
Android 앱은 `app/src/main/assets/magazine/<page.id>.jpg`(png, webp도 가능) 파일이 있으면 코드 변경 없이 그 사진을 사용한다. 사진을 주면 각 가구의 상대 좌표만 다시 잡으면 된다.

사진이 오기 전까지는 UI Kit 안에서 로고와 홍보 문구가 없는 사진 조각만 골라 임시로 쓰고 있다. 화면은 인쇄된 잡지 지면처럼 제호, 제목, 사진 판, 사진 설명, `이 화보의 가구` 크레딧 순서로 구성했다.
