...
- 전체 감상 중 재생 시작 자세에서 Roll이 좌우 어느 방향이든 8도에 도달하면 즉시 AR 탐색으로 복귀한다.
- Phone·Glass HUD는 화면 폭을 활용한 고대비 Compass Tape와 조준 영역을 비우는 좌하단 원형 Artificial Horizon을 공통 표시한다. 작은 원 안의 지평선은 현재 Pitch에 따라 이동하고 Roll에 따라 회전하며, Glass 전체 감상에서는 원 둘레의 좌우 8도 종료 눈금을 함께 표시한다.
...
- 현재 Demo는 8도 Roll 기울임 또는 영상 종료 시 AR로 복귀한다. 실제 제품에서는 Target Hardware의 Pose와 입력 특성에 맞춰 임곗값을 다시 검증한다.
