...
- 전체 감상 중 재생 시작 자세에서 Roll이 좌우 어느 방향이든 15도에 도달하면 즉시 AR 탐색으로 복귀한다.
- Phone·Glass HUD는 화면 폭을 활용한 고대비 Compass Tape와 조준 영역을 비우는 좌하단 원형 Artificial Horizon을 공통 표시한다. 작은 원 안의 지평선은 현재 Pitch에 따라 이동하고 Roll에 따라 회전하며, Glass 전체 감상에서는 원 둘레의 좌우 15도 종료 눈금을 함께 표시한다.
- Demo Preview는 기본 활성화하며 실제 GPS 대신 을지로 타워 107 좌표를 사용한다. `USB`와 `운영` Profile은 Backend Demo Zone을 조회하고 연결 실패 시 Local Moment Stack으로 전환하며, `Demo` Profile은 처음부터 Server 없이 Local Moment Stack을 사용한다. Artificial Horizon의 Roll은 AR Tracking이 시작된 자세를 0도로 보정한다.
- 재생 승인 시 6DoF 공간 고정 Preview를 시야 정면의 3DoF형 Screen으로 전환한다.
- 고개를 멀리 돌리는 정상적인 공간 탐색 행동을 종료 명령으로 사용하지 않는다.
- 현재 Demo는 15도 Roll 기울임 또는 영상 종료 시 AR로 복귀한다. 실제 제품에서는 Target Hardware의 Pose와 입력 특성에 맞춰 임곗값을 다시 검증한다.
