...
현재 도면은 통신 배선과 기호를 검증하기에는 적합하지만 Pipe Dataset은 아니다. 작업지시서의 Pipe 3D화 성공 기준을 유지하려면 배관 도면을 추가해야 한다. ET-1101만 사용할 경우 Phase 1의 첫 3D 대상은 통신 배선 경로 또는 기호 배치로 조정하는 것이 자연스럽다.
