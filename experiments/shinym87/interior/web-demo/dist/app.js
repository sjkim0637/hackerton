...
overlay.onpointermove=event=>{if(!dragState?.item)return;const item=dragState.item;if(item.isBuiltIn){const next=clampBuiltInOffset(item,dragState.offsetX+(event.clientX-dragState.startX)/sceneWidth,dragState.offsetY+(event.clientY-dragState.startY)/sceneHeight);item.offsetX=next.x;item.offsetY=next.y;updateSelectedInfo();render();return;}const before={screenX:item.screenX,screenY:item.screenY,x:item.x,z:item.z},next=screenToFloor(event.clientX,event.clientY);item.screenX=next.x;item.screenY=next.y;updateSpatialState(item);if(isInvalidPlacement(item)){Object.assign(item,before);showPlacementBlocked();return;}updateModel(item);updateSelectedInfo();render();};
overlay.onpointerup=()=>{if(dragState?.item){if(dragState.item.isBuiltIn)saveBuiltInState(dragState.item);else saveState();announce(dragState.item.isBuiltIn?"사진 속 가구를 이동했습니다.":"바닥 평면에 고정해 이동했습니다.");}dragState=null;};overlay.onpointercancel=()=>dragState=null;
